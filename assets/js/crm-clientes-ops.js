(() => {
  ...
  function upgradeStaticControls(){
    document.querySelectorAll('.topbar .topbar-btn').forEach(btn=>{const t=(btn.textContent||'').trim();if(t.includes('🔍')){btn.innerHTML=icon('search');btn.classList.add('icon-only');btn.setAttribute('aria-label','Buscar');}else if(t.includes('⚙')){btn.innerHTML=icon('settings');btn.classList.add('icon-only');btn.setAttribute('aria-label','Configuración');}else if(t==='←'||btn.classList.contains('portal-back-btn')){btn.innerHTML=icon('back');btn.classList.add('icon-only');}});
  ...
  const VIEW_INFO={clientes:{title:'Clientes',desc:'Buscá y administrá toda la base de contactos. Podés filtrar, etiquetar, abrir cada ficha y decidir quién puede recibir comunicaciones.'},campanas:{title:'Campañas',desc:'Prepará mensajes masivos para grupos de clientes. Elegí destinatarios, revisá el contenido y avanzá al envío de forma controlada.'}};
  function addViewIntro(id){const info=VIEW_INFO[id],el=document.getElementById('mainContent');if(!info||!el||el.querySelector('.crm-view-intro'))return;const box=document.createElement('div');box.className='crm-view-intro';box.innerHTML=`<div class="crm-view-intro-icon">${icon(id==='clientes'?'users':'campaign')}</div><div><div class="crm-view-intro-title">${info.title}</div><div class="crm-view-intro-desc">${info.desc}</div></div>`;el.prepend(box);}

  function goView(view,id){
    document.body.dataset.crmOpsView=id;
