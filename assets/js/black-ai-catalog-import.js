(()=>{
  ...
  function classify(desc){
    const d=norm(desc);const features=[];
  ...
  function normalizedRow(raw){
    const desc=raw.Descripcion||raw.descripcion||'';
    const c=classify(desc);
    const price=number(raw.PrecioVentaConIva);
    return {
      source_system:SOURCE,
      source_key:stableKey(raw),
      name:desc.trim(),
      family:c.lensLike?'lens':'catalog',
      design:c.design,
      material:c.material,
      treatment:c.treatment,
      optical_case:c.optical_case,
      supply_mode:c.supply_mode,
      description:desc.trim(),
      currency:'ARS',
      base_price:price,
      is_active:true,
      metadata:{source:{system:SOURCE,id_rubro:raw.IdRubro||null,id_subrubro:raw.IdSubRubro||null,id_subsubrubro:raw.IdSubSubRubro||null,price_includes_vat:true},features:c.features,classification_version:1},
      _review:price===null||(c.lensLike&&!c.optical_case),
      _raw:raw
    };
  }
  ...
  function renderPreview(rows){const s=stats(rows);return `<div class="catalog-import-stats"><div><span>Total</span><strong>${s.total}</strong></div><div><span>Nuevos</span><strong>${s.new}</strong></div><div><span>Modificados</span><strong>${s.modified}</strong></div><div><span>Sin cambios</span><strong>${s.unchanged}</strong></div><div><span>Revisar</span><strong>${s.review}</strong></div></div><div class="catalog-import-note">Super Blue se conserva como tratamiento propio (<b>super_blue</b>), separado del filtro azul estándar. Sus beneficios comerciales se explicarán desde Conocimiento.</div><div class="catalog-import-table-wrap"><table class="catalog-import-table"><thead><tr><th>Estado</th><th>Descripción</th><th>Precio</th><th>Caso</th><th>Modalidad</th><th>Tratamiento</th></tr></thead><tbody>${rows.slice(0,120).map(r=>`<tr><td><span class="catalog-status ${r._status}">${statusLabel(r._status)}</span></td><td>${esc(r.name)}</td><td>${r.base_price==null?'—':money(r.base_price)}</td><td>${esc(r.optical_case||'—')}</td><td>${esc(r.supply_mode||'—')}</td><td>${esc(r.treatment||'—')}</td></tr>`).join('')}</tbody></table>${rows.length>120?`<div class="catalog-import-more">Mostrando 120 de ${rows.length} artículos.</div>`:''}</div>`}
  async function applyImport(file,rows,modal){
    const sb=client();if(!sb)throw new Error('Supabase no disponible.');
    const user=(await sb.auth.getSession()).data.session?.user?.id||null;
    const s=stats(rows);
    const {data:batch,error:batchError}=await sb.from('black_ai_import_batches').insert({source_name:file.name,source_type:'csv',status:'approved',summary:{source_system:SOURCE,...s},created_by:user}).select('id').single();
    if(batchError)throw batchError;
    const now=new Date().toISOString();
    const actionable=rows.filter(r=>r._status!=='unchanged');
    for(let i=0;i<actionable.length;i+=80){
      const chunk=actionable.slice(i,i+80).map(r=>({source_system:r.source_system,source_key:r.source_key,name:r.name,family:r.family,design:r.design,material:r.material,treatment:r.treatment,optical_case:r.optical_case,supply_mode:r.supply_mode,description:r.description,currency:r.currency,base_price:r.base_price,is_active:r.is_active,metadata:{...(r._existing?.metadata||{}),...r.metadata},last_import_batch_id:batch.id,imported_at:now,updated_at:now,updated_by:user}));
      const {error}=await sb.from('black_ai_products').upsert(chunk,{onConflict:'source_system,source_key'});if(error)throw error;
    }
    const importRows=rows.map((r,i)=>({batch_id:batch.id,row_number:i+2,raw_data:r._raw,normalized_data:{source_key:r.source_key,name:r.name,base_price:r.base_price,optical_case:r.optical_case,supply_mode:r.supply_mode,material:r.material,treatment:r.treatment,features:r.metadata.features},row_type:'product',status:r._status==='review'?'pending':'imported',error_message:r._status==='review'?'Clasificación o precio requiere revisión':null}));
    ...
    const {error:finishError}=await sb.from('black_ai_import_batches').update({status:'completed',summary:{source_system:SOURCE,...s,imported:actionable.length,completed_at:now}}).eq('id',batch.id);if(finishError)throw finishError;
    modal.querySelector('.catalog-import-body').innerHTML=`<div class="catalog-import-success"><strong>Importación completada</strong><span>${actionable.length} artículos creados o actualizados · ${s.unchanged} sin cambios · ${s.review} para revisar.</span></div>`;
    const applyBtn=modal.querySelector('[data-import-apply]');if(applyBtn)applyBtn.remove();
  }
  function open(){
    const wrap=document.createElement('div');wrap.className='knowledge-modal-backdrop';
    wrap.innerHTML=`<div class="knowledge-modal catalog-import-modal" role="dialog" aria-modal="true"><div class="catalog-import-head"><div><h3>Importar catálogo</h3><p>Subí la exportación CSV de SINERGIA. Primero vas a revisar los cambios y recién después se actualiza el catálogo.</p></div><button class="catalog-close" type="button">×</button></div><div class="catalog-import-body"><label class="catalog-drop"><input type="file" accept=".csv,text/csv"><strong>Seleccionar CSV de SINERGIA</strong><span>Esperamos: Descripcion, IdSubRubro, IdSubSubRubro, IdRubro y PrecioVentaConIva.</span></label></div><div class="knowledge-form-actions"><button class="btn-secondary" type="button" data-import-cancel>Cancelar</button></div></div>`;
    ...
    const input=wrap.querySelector('input[type=file]');input?.addEventListener('change',async()=>{const file=input.files?.[0];if(!file)return;const body=wrap.querySelector('.catalog-import-body');body.innerHTML='<div class="catalog-import-loading">Analizando archivo y comparando con el catálogo actual…</div>';try{const rows=await preview(file);body.innerHTML=renderPreview(rows);const actions=wrap.querySelector('.knowledge-form-actions');actions.insertAdjacentHTML('beforeend','<button class="btn-primary" type="button" data-import-apply>Aplicar importación</button>');actions.querySelector('[data-import-apply]')?.addEventListener('click',async e=>{const btn=e.currentTarget;btn.disabled=true;btn.textContent='Importando…';try{await applyImport(file,rows,modal)}catch(err){btn.disabled=false;btn.textContent='Aplicar importación';alert(`No se pudo importar: ${err?.message||err}`)}})}catch(err){body.innerHTML=`<div class="catalog-import-error"><strong>No se pudo leer el archivo</strong><span>${esc(err?.message||err)}</span></div>`}});
  }
