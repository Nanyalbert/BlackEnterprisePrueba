(()=>{
  ...
  function sinergiaPrice(v){
    ...
    if(comma>=0&&dot>=0){
      const dec=comma>dot?',':'.',th=dec===','?'.':',';
      normalized=raw.split(th).join('').replace(dec,'.');
    }else if(comma>=0){
      const decimals=raw.length-comma-1;
      normalized=(decimals>0&&decimals<=2)?raw.replace(/\./g,'').replace(',','.'):raw.replace(/,/g,'');
    }else if(dot>=0){
      const dots=(raw.match(/\./g)||[]).length,decimals=raw.length-dot-1;
  ...
  function classify(desc){
    ...
    const lensLike=/ORGANICO|MINERAL|POLICARBONATO|ALTO INDICE|BIFOCAL|MULTIFOCAL|OCUPACIONAL|KODAK|CRIZAL|BLUE|FOTOCROM|LENTE|CRISTAL/.test(d);
    ...
    const idx=d.match(/\b1\.(49|50|53|56|59|60|61|67|70|74)\b/);if(idx&&!material)material=idx[0];
    ...
    if(isSuper)features.push('super_blue');else if(isBlue4k)features.push('black_blue_4k');else if(isBlue)features.push('blue_filter');
    if(isPhoto)features.push('photochromic');if(isAr)features.push('antireflective');

    let treatment='none';
    if(isSuper)treatment='super_blue';else if(isBlue4k)treatment='black_blue_4k';else if(isPhoto&&isBlue)treatment='photochromic_blue';else if(isPhoto)treatment='photochromic';else if(isBlue)treatment='blue_filter';else if(isAr)treatment='antireflective';
  ...
  async function preview(file){
    ...
    const {data,error}=await sb.from('black_ai_products').select('id,source_key,name,family,design,material,treatment,optical_case,supply_mode,base_price,metadata').eq('source_system',SOURCE);if(error)throw error;
  ...
  async function applyImport(file,rows,modal){
    ...
    modal.querySelector('.catalog-import-body').innerHTML=`<div class="catalog-import-success"><strong>Importación completada</strong><span>${actionable.length} artículos actualizados.</span></div>`;modal.querySelector('[data-import-apply]')?.remove();return true;
  }

  function open(){
    const wrap=document.createElement('div');wrap.className='knowledge-modal-backdrop';wrap.innerHTML=`<div class="knowledge-modal catalog-import-modal" role="dialog" aria-modal="true"><div class="catalog-import-head"><div><h3>Importar catálogo</h3><p>Subí el CSV de SINERGIA. Primero se previsualizan los cambios.</p></div><button class="catalog-close" type="button">×</button></div><div class="catalog-import-body"><label class="catalog-drop"><input type="file" accept=".csv,text/csv"><strong>Seleccionar CSV de SINERGIA</strong></label></div><div class="knowledge-form-actions"><button class="btn-secondary" type="button" data-import-cancel>Cancelar</button></div></div>`;document.body.appendChild(wrap);const modal=wrap.querySelector('.catalog-import-modal'),close=()=>wrap.remove();wrap.querySelector('.catalog-close').onclick=close;wrap.querySelector('[data-import-cancel]').onclick=close;
    wrap.querySelector('input[type=file]').onchange=async e=>{const file=e.target.files?.[0];if(!file)return;const body=wrap.querySelector('.catalog-import-body');body.innerHTML='<div class="catalog-import-loading">Analizando archivo…</div>';try{const rows=await preview(file);body.innerHTML=renderPreview(rows);const s=stats(rows),safe=s.new+s.modified,actions=wrap.querySelector('.knowledge-form-actions');if(safe>0){actions.insertAdjacentHTML('beforeend',`<button class="btn-primary" type="button" data-import-apply>Aplicar ${safe} cambios</button>`);actions.querySelector('[data-import-apply]').onclick=async ev=>{const btn=ev.currentTarget;btn.disabled=true;btn.textContent='Importando…';try{const ok=await applyImport(file,rows,modal);if(!ok){btn.disabled=false;btn.textContent=`Aplicar ${safe} cambios`}}catch(err){btn.disabled=false;btn.textContent=`Aplicar ${safe} cambios`;alert(err?.message||err)}}}}catch(err){body.innerHTML=`<div class="catalog-import-error"><strong>Error</strong><span>${esc(err?.message||err)}</span></div>`}};
  }
